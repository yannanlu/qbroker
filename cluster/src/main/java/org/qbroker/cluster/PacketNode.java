...
/* PacketNode.java - a cluster node of a MulticastGroup */
...
/**
 * PacketNode is a cluster node of a MulticastGroup.  The cluster
 * contains a number of PacketNodes communicating with each other via
 * DatagramPackets.  One of the nodes is the manager that supervises
 * the entire group or cluster.  The rest of nodes are workers
 ...
 * communication path.  Currently, ExtraReceiver only supports two of the
 * schemes, comm or udp.  In case of comm, an RS232 connection is required.
 * It means the cluster size will be limited to 2.  In case of udp, it requires
 * a separate subnet as the secondary communication path.  Also the URI of
...
public class PacketNode implements ClusterNode, Runnable {
    private String name;
    ...
    private StreamReceiver rcvr = null;
    private PacketReceiver extraRcvr = null;
    ...
    public PacketNode(Map props) {
        Template template;
        Object o;
        URI u;
        String uri, groupIP, inf;
        ...
        if ((groupIP = u.getHost()) == null || groupIP.length() == 0)
            throw(new IllegalArgumentException("no group IP specified in URL"));

        try {
            address = InetAddress.getByName(groupIP);
        }
        catch (UnknownHostException e) {
            throw(new IllegalArgumentException("unknown host: " + groupIP));
        }

        if (! address.isMulticastAddress())
            throw(new IllegalArgumentException("not a multicast IP: "+groupIP));
        ...
        if ((o = props.get("MaxGroupSize")) != null)
            maxGroupSize = Integer.parseInt((String) o);
        else
            maxGroupSize = 2;
        ...
        if ((o = props.get("TimeToLive")) != null)
            ttl = 1000*Integer.parseInt((String) o);
        else
            ttl = -1;

        groupURI = groupIP + ":" + port;
        if (ttl > 0)
            cache = new QuickCache(groupURI, QuickCache.META_DEFAULT,
                maxRetry * sessionTimeout + ttl, 1024);

        if (inf != null)
            group = new MulticastGroup(groupIP, port, 8192, -1, 1, inf);
        else
            group = new MulticastGroup(groupIP, port);

        socket = new UDPSocket(null, 0, bufferSize);
        if (timeout > 0) // socket receive() blocks if timeout = 0
            socket.setTimeout(timeout);
        address = group.getInetAddress();

        nodeList = new AssetList(groupURI, maxGroupSize);
        try {
            myURI = InetAddress.getLocalHost().getHostAddress() +
                ":" + socket.getLocalPort();
        }
        catch (UnknownHostException e) {
            myURI = socket.getLocalAddress().getHostAddress() +
               ":" + socket.getLocalPort();
        }
        ...
        if ((o = props.get("ExtraReceiver")) != null && o instanceof Map) try {
            ...
            if ("comm".equals(u.getScheme())) { // serialport
                ...
                map.put("Offtail", "1");
                rcvr = new StreamReceiver(map);
                rcvrType = RCVR_COMM;
            }
            else if ("udp".equals(u.getScheme())) { // multicast
                InetAddress a;
                extraURI = u.getHost() + ":" + u.getPort();
                map.put("Operation", "listen");
                map.put("Mode", "daemon");
                map.put("XMode", "0");
                map.put("TextMode", "1");
                map.put("Partition", "0,0");
                extraRcvr = new PacketReceiver(map);
                a = extraRcvr.getAddress();
                if (!a.isMulticastAddress())
                    throw(new IllegalArgumentException(
                        "not a multicast IP: "+extraURI));
                rcvrType = RCVR_UDP;
            }
            else { // not supported
                rcvr = null;
    ...
    public void run() {
        ...
        if ("mc_group".equals(threadName)) {
            uri = groupURI;
            new Event(Event.INFO, name + ": " + uri + " started").send();
            while (keepRunning(xq) && group != null) {
                try {
                    group.receive(xq);
                }
                catch (Exception e) {
        ...
        else if ("udp_socket".equals(threadName)) {
            uri = myURI;
            new Event(Event.INFO, name + ": " + uri + " started").send();
        ...
        else if ("rcvr_comm".equals(threadName)) { // for extra comm rcvr
            uri = extraURI;
            new Event(Event.INFO, name + ": " + uri + " started").send();
        ...
        else if ("rcvr_group".equals(threadName)) { // for extra udp rcvr
            uri = extraURI;
            new Event(Event.INFO, name + ": " + uri + " started").send();
            while (keepRunning(xq) && hasExtra) {
                try {
    ...
    private void initialize() {
        ...
        if (hasExtra) {
            ...
            if (rcvrType == RCVR_UDP) { // set extra multicast addess
                myBean.setExtraAddress(extraRcvr.getAddress());
                myBean.setExtraPort(extraRcvr.getPort());
            }
        }
        send(packet);
        ...
        while ((xq.getGlobalMask() & XQueue.KEEP_RUNNING) > 0) {
            ...
            if ((o = xq.browse(cid)) == null) { // null packet
                ...
                uri = packet.getAddress().getHostAddress()+":"+packet.getPort();
                isJMS = false;
            }
            else try { // TextEvent
                String str = ((TextEvent) o).getText();
            ...
            if (!nodeList.containsKey(uri)) { // only process HSHAKE packets
                ...
                if (isJMS) { // make sure address is set for 1st packet
                    ...
                    else if (o != null && o instanceof TextEvent) { // RCVR_UDP
                        String str;
                        a = null;
                        p = 0;
                        str = ((TextEvent) o).getAttribute("UDP");
                        if (str != null && (i = str.indexOf(":")) > 0) try {
                            a = InetAddress.getByName(str.substring(0, i));
                            p = Integer.parseInt(str.substring(i+1));
                        }
                        catch (Exception e) {
                        }
                        if (a != null && p > 0) { // set extra address and port
                            pBean.setExtraURI(str);
                            pBean.setExtraAddress(a);
                            pBean.setExtraPort(p);
                            new Event(Event.INFO, name + ": extra uri of " +
                                str + " is set for " + uri).send();
                        }
                        else {
                            new Event(Event.WARNING, name +": failed to parse "+
                                "extra uri '" + str + "' for " + uri).send();
                        }
                    }
                    else {
    ...
    private void send(DatagramPacket packet) {
        ...
        if (hasExtra) try {
            ...
            switch (rcvrType) { // try to send to the extra destination
              case RCVR_COMM:
                i = rcvr.send(new String(buffer) + "\n");
                if (i < 0)
                    new Event(Event.WARNING, name +
                        ": failed to send packet: " + i).send();
                break;
              case RCVR_UDP:
                a = packet.getAddress();
                i = packet.getPort();
                if (a != null && a.isMulticastAddress()) {
                    p = new DatagramPacket(buffer, 0, buffer.length,
                        myBean.getExtraAddress(), myBean.getExtraPort());
                    extraRcvr.send(p);
                }
                else if (a != null && i > 0) {
                    String uri = a.getHostAddress() + ":" + i;
                    PacketNodeBean pBean = (PacketNodeBean) nodeList.get(uri);
                    if (pBean != null && pBean.getExtraURI() != null) {
                        p = new DatagramPacket(buffer, 0, buffer.length,
                            pBean.getExtraAddress(), pBean.getExtraPort());
                        extraRcvr.send(p);
                    }
                }
                break;
    ...
    private void examine() {
        ...
        while (keepRunning(xq)) {
            ...
            else if (o instanceof DatagramPacket) { // primary or relay packet
                packet = (DatagramPacket) o;
                uri = packet.getAddress().getHostAddress()+":"+packet.getPort();
                isJMS = false;
            }
            ...
            if (!nodeList.containsKey(uri)) { // HSHAKE or RELAY packets
                int role;
                if (groupURI.equals(uri)) { // outgoing packet for relay
                    ...
                    if (i < 0) { // failed to add a newbie
                        ...
                        if (myBean.getRole() == 0) { // I am the master
                            ...
                            packet = new DatagramPacket(
                                strBuf.toString().getBytes(),0,strBuf.length(),
                                myBean.getAddress(), myBean.getPort());
                            myBean.setPacket(packet);
                            send(packet);
                            display(1, DEBUG_EXAM, packet);
                        }
...
*/
          ...
          case PacketNodeBean.MCG_HSHAKE:
            ...
            if (status == NODE_NEW) { // newbie
                ...
                if (myRole == 0 && role < 0) { // assign new role
                    ...
                    // in case someone does not know me
                    strBuf = new StringBuffer(currentTime + " " + getNextSid());
                    strBuf.append(" " + PacketNodeBean.MCG_HSHAKE);
    ...
    public void start(XQueue out, int begin, int len) {
        ...
        s.setDaemon(true);
        s.start();
        if (hasExtra) switch (rcvrType) {
          case RCVR_COMM:
            thr = new Thread(this, "rcvr_comm");
            thr.setPriority(Thread.NORM_PRIORITY);
            thr.setDaemon(true);
            thr.start();
            break;
          case RCVR_UDP:
            thr = new Thread(this, "rcvr_group");
            thr.setPriority(Thread.NORM_PRIORITY);
            thr.setDaemon(true);
            thr.start();
            break;
    ...
    public static void main(String args[]) {
        Thread c;
        int i, number = 10, retry = 3, timeout = 30, port = -1;
        int bufferSize = 8192, sleepTime = 2, waitTime = 100;
        int heartbeat = 10, debug = 0;
        String group = null, logDir = null;
        PacketNode node;
        ...
        props.put("MaxGroupSize", String.valueOf(number));
    ...
    private static void printUsage() {
        System.out.println("PacketNode Version 2.0 (written by Yannan Lu)");
        System.out.println("PacketNode: a Node of the MulticastGroup for clustering");
        System.out.println("Usage: java org.qbroker.cluster.PacketNode -g hostname -p port");
        System.out.println("  -?: print this message");
        System.out.println("  -g: multicast address");
        ...
        System.out.println("  -n: size of the cluster (default: 10)");
