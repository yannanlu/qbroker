...
/* SerialPortNode.java - a cluster node for 2 on serial port */
 ...
 * The cluster contains only 2 SerialPortNodes communicating with each
 * other via serial line.  One of the nodes is the manager that
 * supervises the entire group or cluster.  The other node is the worker
 * reporting to the manager and watching its behavior closely.  In
 * case that the manager quits or the node misses certain number of
 * heartbeats from the manager, the worker node will take over to be
 * the new manager and resumes to work.  Any new node can join the
...
public class SerialPortNode implements ClusterNode, Runnable {
    private String name;
    ...
    public SerialPortNode(Map props) {
        ...
        props.put("XMode", "0");
        props.put("TextMode", "1");
        props.put("Partition", "0,0");
        props.put("EOTBytes", "0x0a");
        props.put("Offtail", "1");
        rcvr = new StreamReceiver(props);
    ...
    public void run() {
        ...
        if ("comm_port".equals(threadName)) {
            uri = groupURI;
            new Event(Event.INFO, name + ": " + uri + " started").send();
            while (keepRunning(xq) && rcvr != null) {
                try {
                    rcvr.receive(xq, 0);
                }
                catch (Exception e) {
    ...
    private void initialize() {
        ...
        while ((xq.getGlobalMask() & XQueue.KEEP_RUNNING) > 0) {
            ...
            if ((o = xq.browse(cid)) == null) { // null packet
                ...
                if ((address = packet.getAddress()) != null)
                    uri = address.getHostAddress() + ":" + packet.getPort();
                else // for relay packet
                    uri = groupURI;
            }
            else try { // TextEvent
    ...
    /** sends the packet to destinations */
    private void send(DatagramPacket packet) {
        ...
        if ((i = rcvr.send(str + "\n")) < 0) {
            new Event(Event.WARNING,name+": failed to send packet: "+i).send();
        }
    }
    ...
    public int relay(Event data, int waitTime) {
        ...
        DatagramPacket packet = new DatagramPacket(strBuf.toString().getBytes(),
            0, strBuf.length());
        if (waitTime <= 0)
        ...
        while ((i = browser.next()) >= 0) { // search for all nodes
            ...
            switch (status) {
              case NODE_RUNNING:
                ...
                strBuf.append(" " + status + " " + uri);
                packet = pBean.getPacket();
                packet.setData(strBuf.toString().getBytes(), 0,strBuf.length());
                send(packet);
              ...
              case NODE_TIMEOUT:
                if (currentTime - tm < sessionTimeout + sessionTimeout) {
                    ...
                    strBuf.append(" " + status + " " + uri);
                    packet = pBean.getPacket();
                    packet.setData(strBuf.toString().getBytes(), 0,
                        strBuf.length());
                    send(packet);
                    n ++;
                    display(1, DEBUG_CHECK, packet);
...
*/
    ...
    private DatagramPacket respond(long currentTime, int type,
        ...
        switch (type) {
          ...
          case PacketNodeBean.PTP_CONFRM:
            ...
            else if (myRole != 0 && // master timeout has been confirmed
                (status == NODE_RUNNING || status == NODE_STANDBY) &&
    ...
    public static void main(String args[]) {
        ...
        props.put("BaudRate", "9600");
        props.put("DataBits", "8");
        props.put("StopBits", "1");
        props.put("Parity", "None");

        try {
            node = new SerialPortNode(props);
    ...
    private static void printUsage() {
        System.out.println("SerialPortNode Version 2.0 (written by Yannan Lu)");
        System.out.println("SerialPortNode: a Node of the SerialPortReceiver for clustering");
        System.out.println("Usage: java org.qbroker.cluster.SerialPortNode -p comm_port");
        System.out.println("  -?: print this message");
        System.out.println("  -p: comm port device");
        ...
        System.out.println("  -n: size of the cluster (default: 2)");
