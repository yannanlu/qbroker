...
/* FTPLoginModule.java - a LoginModule using FTP for authentications */
...
public class FTPLoginModule implements LoginModule {
    ...
    public boolean login() throws LoginException {
        ...
        if (debug) {
            new Event(Event.DEBUG, "[FTPLoginModule] username: " +
                username).send();
            new Event(Event.DEBUG, "[FTPLoginModule] uri: " +
                uri).send();
        }
        ...
        try {
            ph.put("User", username);
            ph.put("Password", new String(password));
            FTPTester reporter = new FTPTester(ph);
        ...
        if ("0".equals(rc)) {
            succeeded = true;
            if (debug)
                new Event(Event.DEBUG, "[FTPLoginModule] authentication " +
                    "succeeded").send();
        }
        else {
            succeeded = false;
