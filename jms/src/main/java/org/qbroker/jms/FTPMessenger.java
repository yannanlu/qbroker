...
/* FTPMessenger.java - an FTP messenger for JMS messages */
 ...
 * files from FTP servers. The method of download() is used for synchronous
 * requests.  The method of store() is used for asynchronous storing content
 * to remote FTP servers. The method of copy() is for copying a file from
 * one FTP server to anther. Both upload() and pickup() are for large file
...
public class FTPMessenger extends FTPConnector {
    ...
    private boolean closeAtEOT = false;
    ...
    public FTPMessenger(Map props) throws IOException {
        super(props);
        ...
        if ((o = props.get("CloseAtEOT")) != null && // one tx per connection
    ...
    public void retrieve(XQueue xq) throws IOException, JMSException {
        ...
        for (i=0; i<=retry; i++) {
            ...
            if ((mask & XQueue.KEEP_RUNNING) > 0 &&
                ...
                xq.cancel(sid);
                new Event(Event.WARNING, "aborted to retrieve " + path +
                    " from " + uri + " due to being disabled").send();
                return;
            }
            try {
     ...
     * and downloads the file via ftp.  If LocalStore is defined in the
    ...
    public void download(XQueue xq) throws IOException, TimeoutException,
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            if (localFilename != null && localFilename.length() > 0) {
                ...
                for (i=0; i<=retry; i++) {
                    try {
                        reply = ftpGet(new File(localFilename), filename);
                    }
                    catch (Exception e) {
                        reply = "" + e.toString();
                    }
                ...
                for (i=0; i<=retry; i++) {
                    try {
                        if (isBytes)
                            reply = ftpGet(out, filename);
                        else
                            reply = ftpGet(msgBuf, filename);
                    }
                    catch (Exception e) {
                        reply = "" + e.toString();
                    }
            ...
            if (reply != null) {
                ...
                xq.remove(sid);
                new Event(Event.ERR, "failed to download " + filename+" from "+
                    uri + " with " + i + " retries: " + reply).send();
                if (!isLocal) {
                    if (isBytes) {
                        out.close();
                        out = null;
                ...
                throw(new IOException("failed to download " + filename+" from "+
                    uri + " with " + i + " retries: " + reply));
            }
            else if (closeAtEOT) try { // close at the end of transaction
                close();
            }
            catch (Exception e) {
            }
    ...
    public void upload(XQueue xq) throws IOException, TimeoutException,
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            if (o == null) { // either got a new request or no request
                ...
                if ((rc = getReplyCode()) != 150) {
                    reply = getReplyString();
                    new Event(Event.ERR, "failed to open stream for " +
                        filename + " with " + rc + ": " + reply).send();
                }
            }
            ...
            if (closeAtEOT) try { // close at the end of transaction
                close();
            }
            catch (Exception e) {
            }
     ...
     * in it and gets the list via ftp.  It puts the list of files into the
    ...
    public void list(XQueue xq) throws IOException, TimeoutException,
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            if (list == null) {
                ...
                new Event(Event.ERR, "failed to list " + dirname + " on " +
                    uri).send();
                throw(new IOException("failed to list " +dirname +" on "+uri));
            }
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            if (localFilename == null || localFilename.length() <= 0) {
                ...
                for (i=0; i<=retry; i++) {
                    try {
                        if (isBytes)
                            reply = ftpPut(in, filename);
                        else
                            reply = ftpPut(msgStr, filename);
                    }
                    catch (Exception e) {
                        reply = "" + e.toString();
                    }
                    ...
                    if (reply == null)
                        break;
                    if (i == 0)
                        new Event(Event.WARNING, "failed to store " + filename +
                            " on " + uri + ": " + reply).send();
                    mask = xq.getGlobalMask();
                    if ((mask & XQueue.KEEP_RUNNING) > 0 &&
                        (mask & XQueue.STANDBY) > 0) { // temporarily disabled
                        xq.putback(sid);
                        new Event(Event.WARNING, "aborted to store " + filename+                            " on " + uri + " due to being disabled").send();
                        if (isBytes)
                            in.close();
                        return;
                    }
     ...
     * FileName to the remote file in LocalStore via ftp.
     *<br><br>
     * It will always try to retrieve the URL from the field specified by
    ...
    public void copy(XQueue xq) throws IOException, TimeoutException,
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            target = null;
            try { // instantiate the target from the JSON content
                StringReader ins = new StringReader(msgStr);
                Map ph = (Map) JSON2Map.parse(ins);
                ins.close();
                for (i=0; i<=retry; i++) {
                    try {
                        target = new FTPConnector(ph);
                    }
                    catch (Exception e) {
                        if (i == 0)
    ...
    private String putAFile(File localFile, String filename, int retry) {
        ...
        for (i=0; i<=retry; i++) {
            try {
                reply = ftpPut(localFile, filename);
            }
            catch (Exception e) {
                reply = "" + e.toString();
            }
