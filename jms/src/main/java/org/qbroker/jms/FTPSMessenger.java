...
/* FTPSMessenger.java - an FTPS messenger for JMS messages */
...
public class FTPSMessenger extends FTPSConnector {
    ...
    public FTPSMessenger(Map props) throws IOException {
        super(props);
     ...
     * and downloads the file via ftps.  If LocalStore is defined in the
    ...
    public void download(XQueue xq) throws IOException, TimeoutException,
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            if (localFilename != null && localFilename.length() > 0) {
                ...
                for (i=0; i<=retry; i++) {
                    try {
                        reply = ftpsGet(new File(localFilename), filename);
                    }
                    catch (Exception e) {
                        reply = "" + e.toString();
                    }
                ...
                for (i=0; i<=retry; i++) {
                    try {
                        if (isBytes)
                            reply = ftpsGet(out, filename);
                        else
                            reply = ftpsGet(msgBuf, filename);
                    }
                    catch (Exception e) {
                        reply = "" + e.toString();
                    }
    ...
    public void list(XQueue xq) throws IOException, TimeoutException,
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            for (i=0; i<=retry; i++) {
                try {
                    list = ftpsList(dirname, FTP_FILE);
                }
                catch (Exception e) {
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            if (localFilename == null || localFilename.length() <= 0) {
                ...
                for (i=0; i<=retry; i++) {
                    try {
                        if (isBytes)
                            reply = ftpsPut(in, filename);
                        else
                            reply = ftpsPut(msgStr, filename);
                    }
                    catch (Exception e) {
                        reply = "" + e.toString();
                    }
    ...
    private String putAFile(File localFile, String filename, int retry) {
        ...
        for (i=0; i<=retry; i++) {
            try {
                reply = ftpsPut(localFile, filename);
            }
            catch (Exception e) {
                reply = "" + e.toString();
            }
