...
/**
 * JSONFormatter formats JSON payload and message properties It supports
 * various operations, such as get, set, parse, merge, union, select, remove,
 * first, last, sort and uniq.
 *<br/>
...
public class JSONFormatter {
    ...
    private static java.lang.reflect.Method getPropertyID = null;
    private static java.lang.reflect.Method getProperty = null;
    private static java.lang.reflect.Method setProperty = null;
    ...
    /**
     * returns a formatted string out of the message and the json payload with
     * the given template
     */
    public static String format(Map obj, Message msg, Template template,
        ...
        for (i=0; i<n; i++) {
            ...
            else if ((key = getPropertyID(field)) != null)
    ...
    /**
     * returns a formatted string out of the message and the json text with
     * the given template
     */
    ...
    private static String getPropertyID(String key) throws JMSException {
        ...
        if (getPropertyID == null)
            throw(new JMSException("getPropertyID not initialized yet"));
        try {
            value = (String) getPropertyID.invoke(null, key);
        }
        catch (Exception e) {
            throw(new JMSException("getPropertyID failed: " + e.toString()));
        }
        return value;
    }

    private static String getProperty(String key,Message msg) throws JMSException {
        String value = null;
        if (getProperty == null)
            throw(new JMSException("getProperty not initialized yet"));
        try {
            value = (String) getProperty.invoke(null, key, msg);
        }
        catch (Exception e) {
            throw(new JMSException("getProperty failed: " + e.toString()));
        }
        return value;
    }

    private static int setProperty(String key, String value, Message msg)
        throws JMSException {
        Integer ic = new Integer(-1);
        if (setProperty == null)
            throw(new JMSException("setProperty not initialized yet"));
        try {
            ic = (Integer) setProperty.invoke(null, key, value, msg);
        }
        catch (Exception e) {
            throw(new JMSException("setProperty failed: " + e.toString()));
        }
        return ic.intValue();
    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            msg = EventUtils.newTextEvent();
            Iterator iter = params.keySet().iterator();
            while (iter.hasNext()) { // set parameters
                str = (String) iter.next();
                if (str == null || str.length() <= 0)
                    continue;
                ((Event) msg).setAttribute(str, (String) params.get(str));
            }
    ...
    private static void printUsage() {
        ...
        System.out.println("Usage: java org.qbroker.json.JSONFormatter -f jsonfile -r rulefile");
    ...
    static {
        try {
            Class<?> cls = Class.forName("org.qbroker.jms.MessageUtils");
            getPropertyID =
                cls.getMethod("getPropertyID", new Class[]{String.class});
            getProperty = cls.getMethod("getProperty",
                new Class[]{String.class, Message.class});
            setProperty = cls.getMethod("setProperty",
                new Class[]{String.class, String.class, Message.class});
        }
        catch (Exception e) {
            e.printStackTrace();
        }
