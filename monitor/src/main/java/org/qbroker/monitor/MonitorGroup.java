...
public class MonitorGroup {
     ...
     * during the refreshing process only.
     */
    ...
    /**
     * It deletes all removed monitors including the generated ones. It also
     * updates existing montor templates in case generated items are removed.
     * The list contains all the monitors in the new property map of the group.
     * The map of change contains key-null pairs for those removed monitors.
     * Upon success, all the key-null paris will be removed from the map of
     * change.  It returns the number of monitors deleted from the group.
     */
    private int deleteMonitors(List list, Map<String, Object> change) {
        ...
        for (i=0; i<n; i++) {
            ...
            if ((o = change.get(key)) == null) { // null for deletion
                ...
                if (templateList.containsKey(key)) { // for a monitor template
                    temp = (ConfigTemplate) templateList.remove(key);
                    if (temp != null)
                        size = temp.getSize();
                    else
                        size = 0;
                    for (int j=0; j<size; j++) { // remove each generated obj 
                        k += destroyMonitor(temp.getKey(j));
                        new Event(Event.INFO, name + ": " + temp.getKey(j) +
                            " has been removed for " + key + " at " + j).send();
                    }
                    if (temp != null)
                        temp.close();
                    operation = "deleted";
                }
                else if (monitorList.containsKey(key)) { // for a normal monitor
        ...
        // look for existing monitor templates to diff
        for (i=0; i<n; i++) {
            o = list.get(i);
            if (o == null || !(o instanceof Map)) // not a monitor template
                continue;
            ph = (Map) o;
            ...
            if (c == null) // no change on names and list
                ...
                if (o == null)
                    o = new ArrayList();
                else if (o instanceof String) { // dynamic role swapped
                    if (!change.containsKey(key)) {
                        ...
                        for (int j=0; j<size; j++) {
                            ...
                            new Event(Event.INFO, name + ": " + str +
                                " has been removed for " + key +
                                " due to dynamic role swapped").send();
                        }
                    ...
                    if (!change.containsKey(key)) {
                        ...
                        new Event(Event.INFO, name + ": dynamic reporter " +
                            key + " has been reset").send();
                    }
                    o = new ArrayList();
                }
                ...
                for (int j=0; j<size; j++) {
                    ...
                    if (temp.containsItem(str)) {
                        ...
                        k += destroyMonitor(str);
                        new Event(Event.INFO, name + ": " + str +
                            " has been removed from " + key + " at "+j).send();
                    }
                }
                ...
                if (temp.getSize() <= 0 && !temp.isDynamic()) { // empty
                    ...
                    if ((o = c.get("Template")) != null) { // renamed
                        ...
                        if (temp.resetTemplate((String) o)) { // reset
                            ...
                            new Event(Event.ERR, name + " Config: " +
                                "failed to reset template for " + key).send();
                    }
                }
            }
            else if (!change.containsKey(key)) { // no change on key
    ...
    /**
     * With the new property map, props, of the group and the map of change
     * for the group, it creates or updates all the monitors according to the
     * order specified in the map of props. The map of change should not
     * contain any null value.  Each key-value pair in the map of change is
     * for either a new monitor or an existing monitor with changes.  After it
     * is done, the map of props should have all property maps for the group.
     */
    @SuppressWarnings("unchecked")
    private void updateMonitors(Map props, Map<String, Object> change) {
        ...
        long[] taskInfo;
        int i, j, k, n, id;
        boolean isNewMaster = true, isTemplate = false;
        ...
        for (i=0; i<n; i++) { // loop thru all monitors
            ...
            if (!change.containsKey(key)) { // existing obj with no change
                ...
                if (monitorList.containsKey(key)) { // existing one
                ...
                else if (!isTemplate) { // normal monitor failed or disabled
                ...
                else if (!templateList.containsKey(key)) { // regenerate objs 
                    ConfigTemplate temp = null;
                    try {
                        c = Utils.cloneProperties((Map) o);
                        c.put("Property", props.get(key));
                    ...
                    for (j=0; j<k; j++) { // loop thru all items
                        ...
                        if (monitorList.containsKey(str)) { // duplicated
                            ...
                            new Event(Event.INFO, name + " Config: " +str+
                                " has been "+ operation+" for "+key).send();
                        }
                    }
                }
            ...
            else if (!isTemplate) { // regular obj with changes
                ...
                if (!cachedProps.containsKey(key)) { // new obj
                    ...
                    if ((debug & Service.DEBUG_LOAD) > 0)
                        new Event(Event.DEBUG, name + ": " + key +
                            " has been removed due to reload").send();
                    map.clear();
                    monitor = initMonitor((Map) o, name);
                    if (monitor == null) { // failed
                        removeMonitor(key);
                ...
                if (!templateList.containsKey(key))
                    ...
                    new Event(Event.ERR, name + " Config: " +
                        "duplicated key for monitor template: " + key).send();
                    temp.close();
                    continue;
                }
                if (temp.isDynamic())
                ...
                for (j=0; j<k; j++) { // loop thru all items
                    ...
                    new Event(Event.INFO, name + ": " + str +
                        " has been removed for " + key + " on reload").send();
                }
                temp.close();
                temp = null;
                try {
                ...
                for (j=0; j<k; j++) { // loop thru all items
                    ...
                    if (monitorList.containsKey(str)) { // duplicated
                        ...
                        else if (!h.containsKey(temp.getItem(j))) { // generated
                            taskInfo = new long[TASK_TIME+1];
                            for (int l=0; l<=TASK_TIME; l++)
                                taskInfo[l] = 0; 
                            taskInfo[TASK_TYPE] = 1;
                        ...
                        new Event(Event.INFO, name + " Config: " + str +
                            " has been " + operation + " for " + key +
                            " due to the change of template").send();
                    }
                }
                // cleanup in case h is not empty
                iter = h.keySet().iterator();
    ...
    public int reload(Map<String, Object> change) {
        ...
        if (change.containsKey(basename)) { // for new master
            ...
            if (master != null) {
                ...
                // delete monitors that have been removed
                k = deleteMonitors(list, change);

                if (k > 0 && (debug & Service.DEBUG_DIFF) > 0)
                    new Event(Event.DEBUG, name + ": " + k +
                        " monitors have been deleted").send();
            }
        }
    ...
    /**
     * It removes the monitor of the name from the group and returns the
     * removed object.
     */
    ...
    /**
     * It removes the monitor of the name from the group and destroies the
     * monitor.  It returns 1 for success or 0 otherwise.
     */
     ...
     * a Map upon success or null otherwise.  If the monitor is successfully
     * initialized but it has been disabled, an empty Map will be returned.
     * Otherwise, the returned Map contains all the objects for the monitor.
     */
