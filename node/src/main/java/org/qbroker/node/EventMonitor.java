...
public class EventMonitor extends Node {
    ...
    private int copyCache(QuickCache s, QuickCache t) {
        int n;
        String key;
        Object[] keys;
        if (s == null || s.size() <= 0)
        ...
        for (int i=0; i<n; i++) {
            key = (String) keys[i];
            t.insert(key,s.getTimestamp(key),0,s.getMetaData(key),s.get(key));
        }
        if (t.size() > 0)
    ...
    public int removeRule(String key, XQueue in) {
        ...
        if (id == 0) // can not remove the default rule
            ...
            if (h != null) {
                ...
                if (filter != null)
                    filter.clear();
                EventEscalation a = (EventEscalation) h.remove("Escalator");
                if (a != null)
                    a.clear();
    ...
    public void close() {
        ...
        while((rid = browser.next()) >= 0) {
            ...
            if (rule != null) {
                ...
                if (filter != null)
                    filter.clear();
                escalator = (EventEscalation) rule.remove("Escalator");
                if (escalator != null)
                    escalator.clear();
