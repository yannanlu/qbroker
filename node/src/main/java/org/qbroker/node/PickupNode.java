...
/* PickupNode.java - a MessageNode picking up JMS messages */
 ...
 * as a request to retrieve messages. For each incoming messages, the URI
 * identifies the data source. PickupNode looks it up in the local cache for
 * any XQueue as the transmit queue established for the data source.  If there
 * is an XQueue in the cache for the data source, PickupNode just packs up all
 * the messages in the XQueue into the incoming message body and sends it back
 * as the response. Otherwise, PickupNode will generate a new request with the
 ...
 * thread and the id of the XQueue with the URI as the key.  Therefore,
 * PickupNode is able to receive JMS messages via the receivers from arbitrary
 * data sources based on their URIs and the predefined rulesets on-demand.
 * The received messages will be cached in their XQueues for picking up until
 * they expires.
 * <br/><br/>
 ...
 *<br/><br/>
 * URI is used to identify data sources.  In order to construct the URI
 * for an arbitrary data source, each ruleset contains two sets of
 * format operations.  The first one is the array of templates with the name
 * of URITemplate.  The other is the array of substitutions with the name of
 ...
 * will be routed to failure outlink.  Besides, a ruleset may contain a
 * hash of DefaultProperty as static or dynamic properties for the new
 * receivers. If the URI string is not in the cache, PickupNode will try to
 * resolve all the dynamic variables in the hash of DefaultProperty from the
 * incoming message first. Then it adds the DefaultProperty to the new request
 * for the new receiver. So the extra properties are able to passed over to
 * the receiver pool.
 *<br/><br/>
 * For each new data source, PickupNode creates an Object message as
 * the request containing the URI and the XQueue, as well as the other
 ...
 * receiver thread receiving the messages from the data sources.  The
...
public class PickupNode extends Node {
    ...
    public PickupNode(Map props) {
        ...
        assetList = NodeUtils.initFixedOutLinks(tm, capacity, maxReceiver,
            overlap, name, list);
        NOHIT_OUT = overlap[0];
    ...
    protected Map<String, Object> initRuleset(long tm, Map ph, long[] ruleInfo){
        ...
        if (preferredOutName != null) { // for bypass
            ...
            ruleInfo[RULE_PID] = TYPE_PICKUP;

            if ((o = ph.get("ResultTemplate")) != null)
                rule.put("ResultTemplate", new Template((String) o));
    ...
    public void propagate(XQueue in, XQueue[] out) throws JMSException {
        ...
        boolean isXA = ((in.getGlobalMask() & XQueue.EXTERNAL_XA) > 0);
        ...
        while (((mask = in.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            if (rid != previousRid) {
                ...
                if (ruleInfo[RULE_PID] == TYPE_PICKUP) {
                    rtmp = (Template) rule.get("ResultTemplate");
            ...
            if (i < 0) { // failed to apply filters
                ...
                if (uriStr == null || uriStr.length() <= 0)
                    oid = FAILURE_OUT;
                else if ((oid = assetList.getID(uriStr)) >= 0) {//existing subs
                    ...
                    if (asset == null || asset.length <= ASSET_THR) {
                        ...
                        Thread thr = (Thread) asset[ASSET_THR];
                        xq = (XQueue) asset[ASSET_XQ];
                        outInfo = assetList.getMetaData(oid);
                        if (thr == null) { // rcvr not instantiated yet
                            new Event(Event.WARNING, name +": " + ruleName +
                                " receiver is not ready for " + uriStr).send();
                            oid = FAILURE_OUT;
                        }
                        else { // pick up messages from xq and pack them
                            i = MessageUtils.pack((int) ruleInfo[RULE_MODE],
                                name + ": " + ruleName, rtmp, xq, buffer,
                                inMessage);
                            if (i < 0)
                                oid = FAILURE_OUT;
                            else try {
                                ...
                                MessageUtils.setProperty(rcField,
                                    String.valueOf(i), inMessage);
                            }
                            catch (Exception e) {
                            }
                        }
                        // incremental of pickup requests in OUT_DEQ
                        outInfo[OUT_DEQ] ++;
                    ...
                    asset = (Object[]) assetList.get(oid);
                    xq = new IndexedXQueue(pool.getName() + "_" + oid,
                        (int) outInfo[OUT_CAPACITY]);
                    asset[ASSET_XQ] = xq;
                    asset[ASSET_URI] = xq.getName();
                    asset[ASSET_THR] = null;
                    i = -1;
                    for (int k=0; k<100; k++) { // reserve any empty cell
                    ...
                    if (i >= 0) { // reserved a cell for the request to pool
                        ...
                        if (msg != null) { // send the request
                            ...
                            if (k >= 0) {
                                ...
                                outInfo[OUT_DEQ] ++;
                                try {
                                    MessageUtils.setProperty(rcField, "0",
                                        inMessage);
                                }
                                catch (Exception e) {
                                }
            ...
            else if (oid > POOL_OUT) { // send response back for normal request
                if (isXA) try {
                    inMessage.acknowledge();
                }
                catch (Exception e) {
                    new Event(Event.WARNING, name + ": " + ruleName +
                        " failed to ack msg for " + uriStr).send();
                }
                catch (Error e) {
                    in.remove(cid);
                    Event.flush(e);
    ...
    /**
     * collects receiver threads for new XQueues and returns number of failed
     * receivers
     */
    private int collect(XQueue in, XQueue out, long[] poolInfo) {
        ...
        if (k > 0 && (debug & DEBUG_COLL) > 0)
            new Event(Event.DEBUG, name + " collect: Name OID RID Status " +
                "TTL URI - " + k + " receivers collected from " +
    ...
    /**
     * collects the receiver thread from msg and returns 1 upon success or 0
     * otherwise
     */
    private int collect(long tm, int rid, int oid, XQueue in, ObjectMessage msg,
        ...
        if (bag == null || !bag.containsKey("Thread")) {
            new Event(Event.ERR, name + ": " + ruleList.getKey(rid) +
                " failed to collect receiver " + oid + " for "+ uriStr).send();
            if (bag != null)
                bag.clear();
            assetList.remove(oid);
            new Event(Event.INFO, name + ": " + ruleList.getKey(rid) +
                " removed receiver " + oid + " due to the stopped XQ " +
     ...
     * and the status of the receiver threads, and updates their state info.
    ...
    private int monitor(long currentTime, XQueue in, AssetList assetList) {
        ...
        for (i=0; i<n; i++) {
            ...
            outInfo[OUT_SIZE] = xq.depth();
            if (thr == null) { // rcvr is not instantiated yet
                if ((int) outInfo[OUT_NRULE] < retry) {
                    outInfo[OUT_NRULE] ++;
                    continue;
                }
                else {
                    new Event(Event.WARNING, name + ": receiver " + oid +
                ...
                new Event(Event.WARNING, name + ": thread of receiver " +
            ...
            if ((mask & NOT_STOPPED) == 0) {
                ...
                outInfo[OUT_DEQ] = 0;
                new Event(Event.NOTICE, name + ": receiver " + oid +
            ...
            switch ((int) outInfo[OUT_STATUS]) {
              case NODE_DISABLED:
                if (outInfo[OUT_DEQ] > 0L) { // flow is back up due to requests
                    outInfo[OUT_STATUS] = NODE_RUNNING;
                    outInfo[OUT_QTIME] = currentTime;
                }
                else { // expired
              ...
              case NODE_RUNNING:
                ...
                if (maxIdleTime > 0 && st >= maxIdleTime) { // idled too long
                    outInfo[OUT_QTIME] = currentTime;
                    outInfo[OUT_STATUS] = NODE_DISABLED;
                    if ((debug & DEBUG_UPDT) > 0)
                        new Event(Event.DEBUG, name + ": receiver " + oid +
    ...
    /**
     * stops all dynamic XQs and cleans up associated assets and the rcvrs
     */
