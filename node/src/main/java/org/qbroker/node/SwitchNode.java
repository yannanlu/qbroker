...
public class SwitchNode extends Node {
    ...
    public int removeRule(String key, XQueue in) {
        ...
        if (id == 0) // can not remove the default rule
            ...
            if (ruleInfo[RULE_GID] >= 0) { // update reference count of plugin
                long[] meta = pluginList.getMetaData((int) ruleInfo[RULE_GID]);
                if (meta == null)
                    throw(new IllegalStateException(name + ": " + key +
                        " has no plugin found at " + ruleInfo[RULE_GID]));
                meta[0] --;
                if (meta[0] <= 0) { // reference count is 0, check closer
                    Object[] asset =
                        (Object[]) pluginList.remove((int) ruleInfo[RULE_GID]);
            ...
            if (rule != null && rule.containsKey("Name")) {
                ...
                for (int i=0; i<RULE_TIME; i++) { // update metadata
                    switch (i) {
                      ...
                      case RULE_PEND:
                      case RULE_COUNT:
                        if (tm == meta[RULE_GID]) // same rule type
                            break;
                      default:
                        ruleInfo[i] = meta[i];
                    }
