...
/* JMSPersister.java - a JMS consumer */
...
/**
 * JMSPersister listens to an XQueue and receives JMS messages
 * from it.  It puts the JMS Messages to a JMS Queue as the output.
 * JMSPersister supports flow control and allows object control from
...
public class JMSPersister extends Persister {
    ...
    /**
     * splits content of the incoming message and puts them to the queue
     */
        ...
        while (((mask = xq.getGlobalMask()) & XQueue.KEEP_RUNNING) > 0) {
            ...
            try {
                if (message instanceof TextMessage) {
                    ...
                    qConn.getQueueSender().send(message);
                }
            }
            catch (JMSException e) {
                xq.putback(sid);
